...
// JCM (below already above, but needed for stat() calls)
//#include <errno.h>
//#include <sys/stat.h>
...
#define bbcp_DEFMT "%d %c %lld %o %lld %lx %lx %9s %1025s"
#define bbcp_DEGMT "%d %c %Ld  %o %Ld  %lx %lx %9s %1025s"
...
//
   if (retc = FSp->Stat((const char *)targpath, &Targ)) targetsz = 0;
...
void bbcp_FileSpec::ExtendFileSpec(bbcp_FileSpec* headp)
{
   ...
   for (d = readdir(dirp) ; NULL != d ; d = readdir(dirp))
   {
      ...
      if ((0 != strcmp(".", d->d_name)) && (0 != strcmp("..", d->d_name)))
      {
         ...
         //
         if (0 != FSp->Stat((const char *)(newp->pathname), &(newp->Info)))
         {
            free(newp->filename);
            delete newp;
            continue;
...
//
   if (retc)
      {if (bbcp_Config.Options & bbcp_KEEP) return retc;
       FSp->RM((const char *)targpath);
      }
      else if (bbcp_Config.Options & bbcp_PCOPY) setStat();
              else if (Info.Otype == 'f')
                      FSp->setMode((const char *)targpath, bbcp_Config.Mode);

// Delete the signature file if one exists
//
   if (targsigf) FSp->RM((const char *)targsigf);
...
//
        if ((retc = FSp->setTimes((const char *)targpath,
                                  Info.atime, Info.mtime)))
           act = (char *)"setting time on";
   else if ((retc = FSp->setMode((const char *)targpath, Info.mode)))
           act = (char *)"setting mode on";
   else if (Info.Group
        && (retc = FSp->setGroup((const char *)targpath, 
                                 (const char *)Info.Group)))
...
//
   if (!FSp && !(FSp = bbcp_getFileSystem((const char *)pathname)))
      {char savefn = *filename;
       *filename = '\0';
       FSp = bbcp_getFileSystem((const char *)pathname);
       *filename = savefn;
      }
...
//
   if (!FSp
   || (!(retc = FSp->Stat((const char *)pathname, &Info))
      && Info.Otype == '?')) retc = ENOENT;
...
// Open the sigfile
//
   if ((outfd = open((const char *)targsigf, O_WRONLY|O_CREAT|O_TRUNC,0600))
...
//
   if (bbcp_Config.Options & bbcp_APPEND)
      {if (!bbcp_UFS.Stat((const char *)targsigf)) return Xfr_Fixup();
       if (targetsz == Info.size || targetsz < 0)
          {if (targetsz >= 0) bbcp_Fmsg("Xfr_Done", "File", targpath,
             (char *)"appears to have already been copied; copy skipped.");
           return (Finalize(0) ? -1 : 1);
          }
       // JCM: Stupid to not copy file said wanted to copy because of some prior copy problem that makes append not possible for whatever reason
...
//
   if (!(bbcp_Config.Options & bbcp_FORCE))
      return bbcp_Fmsg("Xfr_Done","File",targpath,(char *)"already exists.");
      else targetsz = 0;
        ...
        {while(*cp && *cp != '/') cp++;
         ...
         if (!same)
            {PS_New = new bbcp_FileSpec();
             PS_New->pathname = PS_New->filename = PS_New->fspec = strdup(pathrltv);
             PS_New->seqno = plen;
             if (PS_New->Stat(1))
...
//
   if ((infd = open((const char *)targsigf, O_RDONLY)) < 0)
...
//
   ...
   ||  strcmp(TSpec.filename, filename))
      {bbcp_Fmsg("Xfr_Fixup", "Source file '", filename, (char *)"' on",
                 hostname, (char *)
...
//
   if (Info.size == targetsz)
      {bbcp_Fmsg("Xfr_Fixup", "File", targpath, (char *)
